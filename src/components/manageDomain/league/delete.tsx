...
const DeleteCentre = ({ id, centreId }: Props) => {
  ...
  async function deleteCentre() {
    try {
      setIsLoading(true);
      const data = await request.delete(`/centre/${centreId}/league/${id}`);
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        action={deleteCentre}
        message="This action means this league will no longer exist. Are you sure you want to delete this league?"
      />
      {toastMessage && (
        <Toast
